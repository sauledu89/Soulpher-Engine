 ...
 * @brief Implementaci�n para la creaci�n, configuraci�n y limpieza de Render Target Views (RTV) en Direct3D 11.
 *
 * @details
 * Un **Render Target View** es la interfaz que permite a Direct3D escribir en una textura como si fuera la pantalla.
 * Normalmente, el RTV principal est� vinculado al **back buffer** de la swap chain, pero tambi�n se pueden crear RTV
 * personalizados para renderizado a texturas (por ejemplo, en efectos de post-proceso o reflejos).
 *
 * @note
 * Entender el RTV es clave en motores gr�ficos, ya que:
 *  - Controla el destino del renderizado.
 *  - Permite trabajar con renderizado diferido (G-Buffers).
 *  - Es esencial en t�cnicas como render-to-texture o renderizado de sombras.
 *
...
#include "DepthStencilView.h"
  ...
  * @return HRESULT     `S_OK` si se crea correctamente, o un c�digo de error si falla.
  *
  * @note
  * Esta versi�n est� pensada para inicializar el RTV principal de la aplicaci�n.
  * Detecta autom�ticamente si la textura usa **MSAA** para configurar la vista como `TEXTURE2DMS`.
  */
 ...
 *
 * @param device        Dispositivo Direct3D 11.
 * @param inTex         Textura de entrada donde se escribir� el render.
 * @param ViewDimension Dimensi�n de la vista (ej. `TEXTURE2D`, `TEXTURE2DARRAY`).
 * @param Format        Formato deseado o `DXGI_FORMAT_UNKNOWN` para inferir.
 * @return HRESULT      `S_OK` si se crea correctamente, o un c�digo de error si falla.
 *
 * @note
 * Esta versi�n permite crear RTV para texturas auxiliares, ideal para:
 *  - Render-to-texture.
 ...
 * @param numViews      N�mero de vistas de render (normalmente 1).
 ...
 *
 * @note
 * �til para casos donde no se requiere profundidad, como renderizado de HUD o post-proceso.
 */
 ...
 *
 * @note
 * Debe llamarse al cerrar la aplicaci�n o al cambiar de render target
